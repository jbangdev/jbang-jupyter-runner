...
async function runFileInTerminal(
  ...
  console.log(
    '[jupyter-jbang-runner] Looking for existing terminal:',
    terminalName
  );
  ...
  for (const widget of widgets) {
    ...
    if (
      ...
      console.log(
        '[jupyter-jbang-runner] ✓ Found existing terminal, reusing it'
      );
      break;
    }
  }
  ...
  implements DocumentRegistry.IWidgetExtension<any, any>
{
  ...
  createNew(widget: any, context: DocumentRegistry.IContext<any>): IDisposable {
    ...
    if (!fileName.endsWith('.java') && !fileName.endsWith('.jsh')) {
      console.log(
        '[jupyter-jbang-runner] Not a Java file, skipping:',
        fileName
      );
      return {
        dispose: () => {},
    ...
    const button = new ToolbarButton({
      ...
      onClick: async () => {
        console.log(
          '[jupyter-jbang-runner] Run button clicked for:',
          context.path
        );
        try {
