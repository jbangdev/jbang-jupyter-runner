...
# Option 2: Just install JupyterLab (minimum required)
# uv pip install jupyterlab
```

**What gets installed:**
- `jupyterlab>=4.0.0,<5` - The JupyterLab application
- `hatchling`, `hatch-nodejs-version`, `hatch-jupyter-builder` - Build tools
- `pytest` and related packages - For running tests (optional)

**Note**: When you run `uv pip install -e .` in Step 3, pip will also automatically install the build dependencies specified in `pyproject.toml`'s `[build-system]` section.
